    ...
    public class UndirectedGraph
    {
        ...
        public List<Node> FindPath(Node start, Node end)
        {
            ...
            while (PQ.Count != 0)
            {
                ...
                if (u == end)
                    break;
                foreach (UndirectedEdge edge in u.Edges)
                {
                    Node thing = edge.OtherNode(u);
                    double newCost = u.NodeCost + edge.Cost;
                    if (newCost < thing.NodeCost)
                    {
                        PQ.DecreasePriority(edge.OtherNode(u), newCost);
                        thing.NodeCost = newCost;
                        thing.Predecessor = u;
                    }
                }
            }
            for (Node a = end; a != start; a = a.Predecessor)
                finalpath.Add(a);
            finalpath.Add(start);
            finalpath.Reverse();
            return finalpath;
