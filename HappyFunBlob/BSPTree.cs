...
namespace HappyFunBlob
{
            ...
            else
            {
                ...
                foreach( Vector3 axis in axes)
                {
                    ...
                    for(int i= 0; i<objects.Count; i++)
                    {
                        ...
                        {
                            Plane newPlane = SplittingPlane(objects[i-1], objects[i], axis);
                            if(SplitsAt(objects, i, newPlane))
                            {
                                bestPlane = newPlane;
                                bestIndex = i;
                ...
                if(bestScore == 0)
                    throw new NoSplitAxisException();
    ...
    public class BSPTreeInteriorNode : BSPTree
    {
        ...
        public override void DoAllIntersectingObjects(BoundingBox bbox, TreeFunction function)
        {
            PlaneIntersectionType p = SplitPlane.Intersects(bbox);
            if (p == PlaneIntersectionType.Front)
            {
                FrontSubTree.DoAllIntersectingObjects(bbox, function);
            }
            else if (p == PlaneIntersectionType.Back)
            {
