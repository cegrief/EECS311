...
namespace HappyFunBlob
{
    ...
    public class ParticleSystem : DrawableGameComponent
    {
        ...
        public void InsertionDepthSort()
        {
            ...
            {
                temp = particles[i];
                for (j = i - 1; j >= 0 && particles[j].depth > temp.depth; j--)
                {
        ...
        {
            QuicksortRecur(0, particles.Length-1);
        }

        public void QuicksortRecur(int start, int end)
        {
                int i = start;
                int j = end;
                Particle pivot = particles[(start + end) / 2];
                float pvalue = pivot.depth;
                while (i <= j)
                {
                    while (particles[i].depth < pvalue)
                        i++;
                    while (particles[j].depth > pvalue)
                        j--;
                ...
                if (start < j)
                    QuicksortRecur(start, j);
                if (i < end)
                    QuicksortRecur(i, end);
            }
