...
namespace HappyFunBlob
{
    ...
    class MethodCall : SyntaxTree
    {
        ...
        public override object Run(Dictionary dict)
        {
            ...
            for(int i=0; i <Arguments.Length; i++)
                temp[i] = Arguments[i].Run(dict);
            return ObjectExpression.Run(dict).CallMethod(MethodName, temp);
        }
    }
    ...
    class OperatorExpression : SyntaxTree
    {
        ...
        public override object Run(Dictionary dict)
        {
            object[] temp = new object[Children.Length];
            for (int i = 0; i < Children.Length; i++)
                temp[i] = Children[i].Run(dict);
            return Interpreter.GenericOperator(Label, temp);
        }
    }
}
